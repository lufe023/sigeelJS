...
const createConditionServices = (req, res) => {
    ...
    res.status(400).json({
    message: `debe llenar los campos citizenId: que se toma del parametro de la url y conditionDetails que se envia en el body`, citizenId, conditionDetails})
}
    // .then((data) => {res.status(200).json(data)})
    // .catch((err) => {res.status(400).json({ message: err })})
}
...
const createParticipationServices = (req, res) => {
    ...
    inTouchController
    .addParticipationController(citicenID,activityDescription,receiveAt)
    .then((data) => {res.status(200).json(data)})
    .catch((err) => {res.status(400).json({ message: err })})
}
else{
    res.status(400).json({
    message: `debe llenar los campos citicenID, activityDescription, receiveAt que se envia en el body`, citicenID, activityDescription })
}
}
...
const createBenefitServices = (req, res) => {
    ...
    res.status(400).json({
    message: `debe llenar los campos citicenID, activityDescription, receiveAt que se envia en el body`, citicenID, benefitDescription })
}
}
...
const createJobServices = (req, res) => {
    //extracion y declaracion de las variables y los datos a enviar
    const citicenID = req.params.id
    const {institution, position, positionDetails, startedAt, finishAt } = req.body

    if(citicenID && institution, position){
    inTouchController
    .addJobController(citicenID,institution, position, positionDetails, startedAt, finishAt)
    .then((data) => {res.status(200).json(data)})
    .catch((err) => {res.status(400).json({ message: err })})
    }
    else{
