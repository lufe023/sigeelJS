    ...
    if (peopleId) {
        censusControllers
            ...
            .then((result) => {
                if (result[0]) {
                    res.status(200).json({
                        message: `Persona agregada de forma exitosa`,
                    });
                } else {
                    res.status(404).json({
        ...
        res.status(400).json({
            message: "Se debe enviar peopleId de tipo UUID",
        });
    }
};
...
const removePeople = (req, res) => {
    ...
    if (peopleId && leaderId) {
        censusControllers
            ...
            .then((result) => {
                if (result[0]) {
                    res.status(200).json({ message: `Accion exitosa` });
                } else {
                    res.status(404).json({
                        message: "esta peticion no es valida",
                    });
        ...
        res.status(400).json({
            message: "Se debe enviar peopleId y leaderid ambas de tipo UUID",
        });
    }
};
...
const updatePeopleService = (req, res) => {
    ...
    if (data) {
        ...
        res.status(400).json({
            message: "Se debe enviar la cedula en la peticion",
        });
    }
};
...
const transferCensusService = (req, res) => {
    ...
    if (leaderIdA && leaderIdB) {
        userController.getUserById(leaderIdA).then((userA) => {
            if (userA.active) {
                res.status(400).json({
                    message:
                        "El usuario donador debe estar desactivado para poder donar el padroncillo",
                });
            } else {
                censusControllers
                    .transferCensusController(leaderIdA, leaderIdB)
                    .then((data) => {
                        res.status(200).json(data);
                    })
                    .catch((err) => {
        ...
        res.status(400).json({
            message:
                "Se deben elegir dos usuarios para que ocurra la trasferencia",
        });
    }
};
