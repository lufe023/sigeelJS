    ...
    const campains = await Campain.findAll({
        attributes: {
            include: [
                [
                    Sequelize.literal(
                        "(SELECT COUNT(*) FROM Polls WHERE Polls.campain = Campain.id)"
                    ),
                    "encuestasCount",
                ],
                [
                    Sequelize.literal(
                        '(SELECT COUNT(*) FROM Polls WHERE Polls.campain = Campain.id AND (prefered_party IS NOT NULL OR elector_type IS NOT NULL OR president IS NOT NULL OR senator IS NOT NULL OR diputy IS NOT NULL OR mayor IS NOT NULL OR councillor IS NOT NULL OR "districtDirector" IS NOT NULL OR "districtCouncilor" IS NOT NULL ))'
                    ),
                    "pollWithAnyComplete",
                ],
            ],
...
const createCampains = async (data) => {
    try {
        ...
        const peoples = await CensusControllers.getPeoplesByPlaces(
            data.provincia,
            data.municipio
        );
...
const activeCampainController = async (id, active) => {
    ...
    try {
        ...
        return {
            success: true,
            message: "Campaña y encuestas actualizadas exitosamente.",
        };
    } catch (error) {
        ...
        return {
            success: false,
            message: "Error al actualizar campaña y encuestas.",
        };
    }
};
...
const getCampainsByCollegeController = async (collegeId) => {
    const campains = await Census.findAll({
        ...
        attributes: [
            ...
            "Encuestas.Campain.isActive",
        ], // Incluye la columna que necesitas
        include: [
            {
                model: Polls,
                as: "Encuestas",
